...
class SimpleRNN(nn.Module):
    def __init__(self):
        ...
        # Fully Connected Layers
        fc1_in_features = 1660224
        self.rnn1 = nn.RNN(in_features=fc1_in_features, out_features=32)
        self.rnn2 = nn.RNN(in_features=32, out_features=16)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model', help='The desired model', required=True)
    parser.add_argument('-d', '--data_path', help='Base path to data', required=True)
    parser.add_argument('-s', '--seed', help='Seed to ensure standard behavior', default=42, required=False)
    parser.add_argument('-b', '--batch_size', help='Size of the batch', default=16, required=False)
    parser.add_argument('-e', '--epochs', help='Number of training epochs', default=25, required=False)
    ...
    print(f'BEGIN TRAINING - model {args['model']} - seed {args['seed']} - batch_size {args['batch_size']} - epochs {args['epochs']} - data {base_path} - {datetime.datetime.now()}', flush=True)
