...
for i in range(len(df_train.looped_audio)):
    np.save(f'./preprocessed/train/{df_train.file_name[i][:-4]}.npy', df_train.looped_audio[i])
print(f'Training data saved! {datetime.datetime.now()}', flush=True)


for i in range(len(df_val.looped_audio)):
    np.save(f'./preprocessed/val/{df_val.file_name[i][:-4]}.npy', df_val.looped_audio[i])
